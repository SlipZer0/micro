...
const CourseDetailPage: NextPage = () => {
  ...
  const handleCloseModal = useCallback(() => {
    ...
    setIsPlaying(false);
    // redirect to pay with summary
    const amount = liveCounter.toFixed(3);
    router.push(
      `/pay?amount=${amount}&seconds=${watchTime}&title=${encodeURIComponent(course.title)}&courseId=${courseId}`,
    );
  }, [course.title, courseId, liveCounter, watchTime, router]);
